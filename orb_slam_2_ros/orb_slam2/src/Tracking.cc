...
Tracking::Tracking(System* pSys, ORBVocabulary* pVoc, FrameDrawer* pFrameDrawer,
                   Map* pMap, KeyFrameDatabase* pKFDB, const int sensor,
  ...
  // Unpack all the parameters from the parameters struct (this replaces loading
  // in a second configuration file)
...
bool Tracking::Relocalization() {
  ...
  if (!bMatch) {
    mCurrentFrame.mTcw = cv::Mat::zeros(
        0, 0,
        CV_32F);  // this prevents a segfault later
