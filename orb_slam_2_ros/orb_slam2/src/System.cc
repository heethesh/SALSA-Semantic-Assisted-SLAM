...
void System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight,
...
void System::TrackMonocular(const cv::Mat &im, const double &timestamp,
  ...
  cv::Mat Tcw = mpTracker->GrabImageMonocular(im, timestamp, semanticmap);
...
cv::Mat System::DrawCurrentFrame() { return mpFrameDrawer->DrawFrame(); }
...
bool System::SetCallStackSize(const rlim_t kNewStackSize) {
  ...
  if (rlimit.rlim_cur <= kNewStackSize) {
    ...
    if (operation_result != 0) {
      std::cerr << "Setrlimit returned result: " << operation_result
                << std::endl;
      return false;
    }
    return true;
...
void System::EnableLocalizationOnly(bool localize_only) {
  ...
  std::cout << "Enable localization only: "
            << (localize_only ? "true" : "false") << std::endl;
}
...
bool System::LoadMap(const string &filename) {
  ...
  if (!in) {
    cerr << "Cannot open map file: " << map_file
