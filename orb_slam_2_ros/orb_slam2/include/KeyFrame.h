...
class KeyFrame {
 ...
 public:
  ...
  const std::vector<float> mvScoreDynamic;
  const std::vector<float> mvScoreRepeatable;

  // TODO(ayush): Store frame level score if needed
  // TODO(ayush): move this to protected block if not thread-safe
  float mfScoreKeyFrame;
